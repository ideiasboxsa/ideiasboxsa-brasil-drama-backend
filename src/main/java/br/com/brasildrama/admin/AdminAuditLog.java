...
interface AdminAuditLogRepository extends JpaRepository<AdminAuditLog,UUID>{java.util.List<AdminAuditLog> findAllByOrderByCreatedAtDesc(PageRequest page);}
...
class AdminAuditApi {
    ...
    @GetMapping ResponseEntity<?> latest(Principal principal){
        if(principal==null)return ResponseEntity.status(403).build();
        AdminOperator actor; try{actor=operators.findById(UUID.fromString(principal.getName())).orElse(null);}catch(IllegalArgumentException ex){actor=null;}
        if(actor==null||!actor.active||actor.role!=AdminRole.SUPER_ADMIN)return ResponseEntity.status(403).build();
        var result=logs.findAllByOrderByCreatedAtDesc(PageRequest.of(0,100)).stream().map(log->{
            var source=operators.findById(log.actorOperatorId).orElse(null);
